<template>
  <div class="map-model-model-form-item">
    <el-form-item :label="props.formItem.label" :prop="props.formItem.property">
      <div class="map-model-model-form-item-container">
        <div class="action-form-item-content" @click="selectKeyDialogVisible=true">
        ...
        <div v-if="props.formItem.map&&props.formItem.map.key.clazz.name" class="model-form-item-content">
          <data-model-table-model-edit-view v-if="formModel[props.formItem.property]"
        ...
        <el-dialog v-if="props.formItem.map&&props.formItem.map.key.clazz.name"
                   v-model="selectKeyDialogVisible"
                   :show-close="false"
                   append-to-body
                   top="15vh"
                   width="60%"
        >
          ...
          <model-select-table v-if="props.formItem.map&&props.formItem.map.key.clazz.name&&selectKeyDialogVisible"
...
import DataModelTableModelEditView
  from "@/views/admin/components/form/components/item/components/DataModelTableModelEditView.vue";
...
const selectKeyDialogVisible = ref<boolean>(false)
const handleSelect = (selection: any) => {
  ...
  selectKeyDialogVisible.value = false
}
onMounted(() => {
...
.map-model-model-form-item {
  .map-model-model-form-item-container {
