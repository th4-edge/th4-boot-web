...
const customizeModelSelectStore: any = {};
(() => {
    const files: any = import.meta.glob(
        "@/views/**/model-select/*ModelSelect.vue"
    );
    for (const key in files) {
        const dir = key.split('/');
        try {
            customizeModelSelectStore[dir[dir.length - 1].replace('ModelSelect.vue', '').toLowerCase()] = defineAsyncComponent(files[key]);
        } catch (e) {
        }
    }
})()
export default customizeModelSelectStore;