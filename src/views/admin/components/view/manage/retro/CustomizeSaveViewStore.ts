...
const customizeSaveViewStore: any = {};
(() => {
    const files: any = import.meta.glob(
        "@/views/**/save-view/*SaveView.vue"
    );
    for (const key in files) {
        const dir = key.split('/');
        try {
            customizeSaveViewStore[dir[dir.length - 3].toLowerCase()] = defineAsyncComponent(files[key]);
        } catch (e) {
        }
    }
})()
export default customizeSaveViewStore;