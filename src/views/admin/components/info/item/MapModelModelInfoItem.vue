<template>
  <div class="model-collection-info-item">
    ...
    <div class="model-collection-info-content">
      <div class="model-table-view-wrapper" v-if="modelValue">
        <map-model-table-view v-if="infoItem.map"
                              :data="modelValue" :ignored-table-columns="ignoredTableColumns"
                              :map="infoItem.map" :span-method="spanMethod" multiple/>
      </div>
...
import MapModelTableView from "@/views/admin/components/info/item/components/table/MapModelTableView.vue";
...
const modelValue = computed(() => {
  ...
  return value ? value[props.infoItem.property] : {key: [], value: []};
})
</script>
